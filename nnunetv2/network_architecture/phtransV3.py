...
import time
from nnunetv2.network_architecture.swin_3Dmodify import *
...
class ConvBlock(nn.Module):
    def __init__(self, input_channels, output_channels,
                 ...
                 mode='axiswise'):  # 'axiswise', 'vanilla', 'pseudo3d'
        super(ConvBlock, self).__init__()
        self.mode = mode.lower()
        assert self.mode in ['axiswise', 'vanilla', 'pseudo3d'], f"Unsupported mode: {self.mode}"

        # ===== 基础卷积 =====
        self.conv1 = nn.Conv3d(input_channels, output_channels, kernel_size=3, padding=1)

        # ===== Dropout =====
        if dropout_op is not None and dropout_op_kwargs is not None and dropout_op_kwargs.get('p', 0) > 0:
        ...
        # ===== 残差卷积 =====
        self.rd_conv = nn.Conv3d(input_channels, output_channels, kernel_size=1) \
            if input_channels != output_channels else None

        # ===== 根据 mode 条件初始化卷积 =====
        if self.mode == 'axiswise':
        ...
        # elif self.mode == 'vanilla':
            self.conv_vanilla = nn.Conv3d(output_channels, output_channels, kernel_size=3, padding=1)
        elif self.mode == 'pseudo3d':
            self.conv_xy = nn.Conv3d(output_channels, output_channels, kernel_size=[1,3,3], padding=[0,1,1])
            self.conv_z  = nn.Conv3d(output_channels, output_channels, kernel_size=[3,1,1], padding=[1,0,0])
    ...
    def forward(self, x):
        ...
        if self.mode == 'axiswise':
            ...
            stack = torch.stack([x_s, x_h, x_w], dim=1)  # [B, 3, C, D, H, W]
            B, N, C, D, H, W = stack.shape

            # Global average pooling along spatial dims -> [B, 3, C]
            gap = stack.mean(dim=[3, 4, 5])  # [B, N, C]

            # MLP for attention, output one weight per direction
            # 这里假设 self.attention_fc 已定义为 nn.Sequential(nn.Linear(C, C//4), nn.ReLU(), nn.Linear(C//4, 1))
            attn = self.attention_fc(gap)  # [B, N, 1]

            # softmax 归一化 -> 每个方向的权重之和为 1
            attn = torch.softmax(attn, dim=1)  # [B, N, 1]

            # reshape for broadcasting -> [B, N, 1, 1, 1, 1]
            attn = attn.view(B, N, 1, 1, 1, 1)

            # weighted sum
            out = (stack * attn).sum(dim=1)  # [B, C, D, H, W]

        elif self.mode == 'vanilla':
            out = self.conv_vanilla(x)
            if self.dropout is not None:
                out = self.dropout(out)

        elif self.mode == 'pseudo3d':
            out = self.conv_xy(x)
            out = self.nonlin(self.instnorm1(out))
            out = self.conv_z(out)
            if self.dropout is not None:
                out = self.dropout(out)
        ...
        out = out + res
        out = self.nonlin(self.instnorm2(out))
        return out
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, num_stage, only_conv, num_only_conv_stage, num_pool, base_num_features, dim, input_resolution, depth, num_heads,
                 ...
                 conv_kernel_sizes=None, conv_pad_sizes=None, pool_op_kernel_sizes=None, basic_block=ConvBlock, 
    ...
    def forward(self, x, skip):
        ...
        # avoid down_or_upsample move the rearrange function down
...
class PHTrans(nn.Module):
    def __init__(self, img_size, base_num_features, num_classes, image_channels=1, only_conv=False, num_only_conv_stage=2, num_conv_per_stage=2,num_transformer_per_stage = 2,
        ...
        for i,head in enumerate(num_heads):
            num_heads[i] = head*2*2**i #default len 4[4, 8, 16, 32]
        conv_pad_sizes = []
    ...
    def forward(self, x):
        ...
        if self.deep_supervision:
            ds = [self.seg_outputs[i](out[-(i+1)]) for i in range(len(out))]
        else:
...
if __name__ == "__main__":
    ...
    dummy_input = torch.randn(1, 1, 64, 192, 160)
